...
# Window Background
bg_img = root.configure(background="#FAF1ED")
...
add_button_fnt = ("Verdana", 20, "bold")
add_button = Button(frame, text="Add", width=5, bg="#795647", fg="white", bd=0, command=addTask)
add_button.place(x=290, y=0)
add_button.configure(font=add_button_fnt)

# To Do List Container
frame1 = Frame(root, bd=1, width=700, height=280,bg="#E1D5CF")
frame1.pack(pady=(240,0))

todolist_container = Listbox(frame1, font=('Comic Sans MS',15,"bold"), width=30,height=10,bg="#E1D5CF",fg="black", cursor="hand2", selectbackground="#9F516D")
...
Button(root, image=delete_icon, bd=0, command=deleteTask).place(x=90, y=550)
delete_text = Label(root, text="Delete", font=("Comic Sans Ms", 12), bg="#FAF1ED").place(x=96, y=620)

# Save task icon
save_icon = PhotoImage(file="images/save.png")
Button(root, image=save_icon, bd=0, command=saveTasks).place(x=170, y=550)
save_text = Label(root, text="Save", font=("Comic Sans Ms", 12), bg="#FAF1ED").place(x=180, y=620)

# Load task icon
load_icon = PhotoImage(file="images/load.png")
Button(root, image=load_icon, bd=0, command=loadTasks).place(x=250, y=550)
load_text = Label(root, text="Load", font=("Comic Sans Ms", 12), bg="#FAF1ED").place(x=260, y=620)
